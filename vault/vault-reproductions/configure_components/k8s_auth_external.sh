# Set up Vault external to k8s cluster that is running app pods
...
# Deploy app pod in k8s cluster (in this case, Vault pod is being used as an application pod but k8s auth isn't configured in this Vault cluster in k8s)
`cd setup`
``./init-primary.sh``
...
kubectl exec -ti -n vault vault-0 -- curl -vv --request POST --data '{"jwt": "'$VAULT_POD_LOCAL_JWT'", "role": "test-role"}' http://$MINIKUBE_HOST_IP:8200/v1/auth/kubernetes/login
